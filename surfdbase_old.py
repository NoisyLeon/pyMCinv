...
class invhdf5(h5py.File):
    ...
    def print_info(self):
        ...
        try:
            outstr  += '--- mask_inv (mask_ray_interp - hybridtomo; mask_inv/mask_LD/mask_HD - raytomo) \n' + \
        ...
        try:
            outstr  += '--- mask_interp (mask_ray - hybridtomo; mask_inv - raytomo) \n' + \
    ...
    def read_inv(self, datadir, ingrdfname=None, factor=1., thresh=0.5, stdfactor=2, avgqc=True, \
        ...
        for grd_id in grdlst:
            ...
            # # # if grd_lon != -164. or grd_lat != 63.5:
            # # #     continue
            temp_mask[ilat, ilon]\
                        = False
            ...
            # --- added 2019/01/16
            grp.create_dataset(name = 'zArr_ensemble', data = vpr.zArr_ensemble)
            grp.create_dataset(name = 'vs_upper_bound', data = vpr.vs_upper_bound)
            grp.create_dataset(name = 'vs_lower_bound', data = vpr.vs_lower_bound)
            grp.create_dataset(name = 'vs_std', data = vpr.vs_std)
            grp.create_dataset(name = 'vs_mean', data = vpr.vs_mean)
    ...
    def get_paraval(self, pindex, dtype='min', ingrdfname=None, isthk=False, depth=5., depthavg=0.):
        """
        ...
        for grd_id in grdlst:
            ...
            #  20181203
                ...
                if dtype != 'std' and dtype != 'sem':
                    ...
                    data[ind_lat, ind_lon]  = paraval[-1] * 2.  #  
            elif pindex == 'vs_std':
                unArr                       = grp['vs_std'].value
                zArr                        = grp['zArr_ensemble'].value
    ...
    def get_filled_paraval(self, pindex, dtype='min', ingrdfname=None, isthk=False, do_interp=False, \
        ...
        minlat      = self.attrs['minlat']
        maxlat      = self.attrs['maxlat']
        data        = self.get_paraval(pindex=pindex, dtype=dtype, ingrdfname=ingrdfname, isthk=isthk, depth=depth, depthavg=depthavg)
    ...
    def paraval_arrays(self, dtype='min', sigma=1, gsigma = 50., verbose=False, depth=5., depthavg=0.):
        """
    ...
    def plot_paraval(self, pindex, is_smooth=True, dtype='avg', sigma=1, gsigma = 50., \
            ingrdfname=None, isthk=False, shpfx=None, outfname=None, clabel='', title='', cmap='cv', \
